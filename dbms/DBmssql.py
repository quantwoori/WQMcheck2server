from cfgr.private import Private
import pymssql
import pandas as pd
import numpy as np
...
class MSSQL:
    ...
    def get_tablename(self, database) -> pd.DataFrame:
    ...
    def select_recent(self, database:str, schema:str, table:str, column:Iterable, recent:int,
                      condition:str=None, orderby:Iterable=['date']) -> [Tuple]:
        """
        :param database: Name of the directory
        :param schema: Name of the schema
        :param table: Name of the table
        :param column: target columns
        :param recent: How many recent rows do you want? ex) 20
        :param condition: any condition that comes after where
        :param orderby: orderby: recent based on what? ex) [str, str]
        """
        columns = ', '.join(column)
        ord = ', '.join(orderby)
        qry = f"SELECT TOP ({recent}) {columns} FROM {database}.{schema}.{table}"

        if condition is not None:
            qry = f"{qry} WHERE {condition}"

        qry = f"{qry} ORDER BY {ord} DESC"
    ...
    def select_db(self, database:str, schema:str, table:str, column:Iterable,
                  distinct=None, condition:str=None, orderby:str=None, groupby:str=None) -> [Tuple]:
        """
